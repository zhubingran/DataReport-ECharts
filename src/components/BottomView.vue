    ...
    wordCloud () {
      ...
      this.wordCloud.forEach((item, index) => {
        totalData.push({
          ...
          count: item.count,
          users: item.user,
          range: `${(item.user / item.count * 100).toFixed(2)}%` // 保留两位小数
        })
      })
      this.totalData = totalData
      this.total = this.totalData.length
      this.renderTable(1)
      this.userCount = this.format(totalData.reduce((result, current) => current.users + result, 0)) // 0是result的初始值， 累加
      this.searchCount = this.format(totalData.reduce((result, current) => current.count + result, 0)) // 0是result的初始值， 累加
